...
CONST_DESCRIPTION_ALGORITHM_TURING_ONE = """        Машина Тьюринга состоит из бесконечной в обе стороны ленты, разделенной на ячейки, и автомата (головки),
который управляется программой. Выражение составляется из набора символов, который называют алфавитом."""

CONST_DESCRIPTION_ALGORITHM_TURING_TWO = """        Программы для машин Тьюринга записываются в виде таблицы, где первые столбец и строка содержат буквы
алфавита и возможные внутренние состояния автомата. 

        Содержимое таблицы представляет собой команды для машины Тьюринга.
Буква, над которой головка находится в данный момент, и внутренне состояние головки определяют,
какую команду нужно выполнить.
"""

CONST_BRIEF_REFERENCE_POST = """        Машина Поста – это абстрактная вычислительная машина,
созданная для уточнения (формализации) понятия алгоритма.
...
содержать метку (1 или любой другой знак).
    2.	головки (каретки), способной передвигаться по ленте на одну ячейку в ту или иную сторону, а также
...
CONST_TRAINER_TUR = """     В верхней части программы находится  поле текста, в которое можно ввести
условие задачи.
...
состоящее из символов алфавита. Нажатием левой кнопки мыши можно открыть меню.
    ...
    В поле "Алфавит" задаются символы алфавита. Пробел добавлять необязательно.
Он добавится к введенным символам автоматически.

    В таблице в нижней части окна набирается программа. В первом столбце
записаны символы алфавита, он заполняется автоматически. В первой строке
перечисляются все возможные состояния. Добавить и удалить столбцы таблицы
(состояния) можно с помощью кнопок, расположенных над таблицей.
    При вводе команды в ячейку таблицы сначала нужно ввести новый символ 
...
Если символ пропущен, по умолчанию он не изменяется.
    Во время выполнения алгоритма справа в поле "Комментарии" появлzются
объяснения к текущему правилу.
    Программа может выполняться непрерывно или по шагам. Команда, которая
сейчас будет выполняться, подсвечивается зеленым фоном.
...
0, 1 и пустота. Вы можете сменить алфавит в верхнем меню в соотвутствующем поле.
